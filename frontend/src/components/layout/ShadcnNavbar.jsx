            ...
            {user && (
              <>
                ...
                {isActive('/dashboard') && (
                  <button
                    onClick={() => {
                      // Get the share button from dashboard and trigger it
                      const shareButton = document.querySelector('[data-share-dashboard]');
                      if (shareButton) {
                        shareButton.click();
                      } else {
                        // Fallback: navigate to dashboard if not already there
                        window.location.hash = '#share-dashboard';
                        alert('Please use the Share Dashboard button on the dashboard page.');
                      }
                    }}
                    className="p-2 text-slate-600 hover:text-slate-900 transition-colors rounded-md hover:bg-gray-100"
